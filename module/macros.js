 ...
 * @return {Promise.<void>}
 */
export const createEternalHungerMacro = async (data, slot) => {
  ...
  const command = `game.eternalhunger.rollItemMacro("${actor.id}", "${item.id}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command,
      flags: { "eternalhunger.itemMacro": true },
    });
  }
...
export const rollItemMacro = async (actorId, itemId) => {
  ...
  const flavor = `${game.i18n.localize("ETERNALHUNGER.Rolling")} damage with ${item.name}`;
